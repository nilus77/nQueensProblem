...
//    }
    ...
    @Override
    public List<Constraint> computeConstraints(Chessboard board, Position queenPosition) {
        var constraints = super.computeConstraints(board, queenPosition);
        List<Constraint> lineBetweenQueensConstraints = board
                .getQueensPositions()
                .stream()
                .map(p -> new QueensInLineConstraint(coordinatesConverter, p, queenPosition, board.getSize()))
                .collect(Collectors.toList());

        lineBetweenQueensConstraints.addAll(constraints);
        return lineBetweenQueensConstraints;
    }
}
